...
class HTTPStore extends Store {
  ...
  readFromState(query) {
    ...
    return this._findOrCreateStoreInstance(path).state;
  }
  ...
  async _forceFetch(path) {
    const { ignoreCache = false } = this._getOptions(path);
  ...
  async _fetch(path, query) {
    ...
    const state = await this._forceFetch(this._getOptions(path).rewritePath(query, path));
    this._set(path, state);
    return state;
  }

  _findOrCreateStoreInstance(path) {
    if(!this.data.has(path)) {
      this.data.set(path, { observers: new Set(), options: {}, state: Store.State.pending({ path }) });
    }
   ...
   */
  _set(path, state) {
    const data = this._findOrCreateStoreInstance(path);
    data.state = state;
   ...
   */
  _getOptions(path) {
    return Object.assign({}, this._options, path ? this._findOrCreateStoreInstance(path).options : {});
  }
